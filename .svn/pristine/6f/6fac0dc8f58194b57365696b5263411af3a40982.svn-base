...
public class HttpTask implements Runnable, Serializable {
    ...
    HttpTask(final ImageRequest imageRequest, ITaskStatusListener listener) {
    ...
    public void run() {
        ...
        try {
            if (DebugOptions.isEnableMemoryCache()) {
                isSuccess = checkMemory(mImageRequest);
                LogUtils.d(TAG, "run: checkMemory returns " + isSuccess);
            }
            if (!isSuccess) {
                if (DebugOptions.isEnableFileCache()) {
                    isSuccess = checkFile(mImageRequest);
                    LogUtils.d(TAG, ">>>>> checkFile returns " + isSuccess);
                }
            }
            if (!isSuccess) {
                isSuccess = httpRequest(mImageRequest.getUrl(), mImageRequest.getShouldBeKilled());
                LogUtils.d(TAG, ">>>>> run: httpRequest returns " + isSuccess);
            }
        } catch (Exception ex) {
            LogUtils.d(TAG, ">>>>>> exception happend:", ex);
            exception = ex;
        } catch (AssertionError error) {
            LogUtils.d(TAG, ">>>>>>assertion error:", error);
            exception = error;
        }
    ...
    /**
     * check Memory whether exist image
     * 
     * @param url
     * @return
     */
    protected boolean checkMemory(ImageRequest request) {
        if (LogUtils.DEBUG) LogUtils.d(TAG, ">> checkMemory()");
        long start = SystemClock.uptimeMillis();
        String url = UrlTool.getModifiedUrlFromRequest(request);
        Bitmap bitmap = mCache.getMemoryBitmap(url);
        if (LogUtils.DEBUG) LogUtils.d(TAG, "<< checkMemory(), bitmap=" + LogUtils.printBitmap(bitmap)
                    + ", time consumed=" + (SystemClock.uptimeMillis() - start));
        if (bitmap != null) {
    ...
    protected boolean checkFile(ImageRequest request) {
        long start = SystemClock.uptimeMillis();
        Bitmap bitmap = mCache.getFileBitmap(request, true);
        if (LogUtils.DEBUG) LogUtils.d(TAG, ">>>>> checkFile(), bitmap=" + LogUtils.printBitmap(bitmap)
                    + ", time consumed=" + (SystemClock.uptimeMillis() - start));
        if (bitmap != null) {
            notifyUISuccess(bitmap);
            return true;
     ...
     */
    private boolean httpRequest(String url, boolean shouldBeKilled) throws ClientProtocolException, IOException, AssertionError {
        long start = SystemClock.uptimeMillis();
        ...
        if (mIsDeprecated && shouldBeKilled) {
            LogUtils.d(TAG, "<< run(): deprecated---httpRequest");
            return isSuccess;
        }
     ...
     * if (mImageRequest.shouldRoundCorner()) {
        ...
        if (bitmap != null) {
            if (LogUtils.DEBUG) LogUtils.d(TAG, "save => recordMemory");
            mCache.recordMemory(mImageRequest, bitmap);
            if (LogUtils.DEBUG) LogUtils.d(TAG, "save => notifyUISuccess");
            notifyUISuccess(bitmap);
            final SoftReference<Bitmap> weakBitmap = new SoftReference<Bitmap>(bitmap);
            ImageProvider.get().addSaveTask(new Runnable() {
                @Override
                public void run() {
...
//                            savedbitmap = BitmapFactory.decodeStream(isBm);
                            mCache.recordFile(mImageRequest, baos.toByteArray());
                            recycled = false;
                        }
                        if (LogUtils.DEBUG) LogUtils.d(TAG, "saveAsync(): recycled=" + recycled);
                    } else {
                        mCache.recordFile(mImageRequest, bytesBuffer);
                    }
